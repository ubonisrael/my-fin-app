...
interface ClearGoalsDialogProps {
    handleClearGoals: () => void
}

export function ClearGoalsDialog({
    handleClearGoals
}: ClearGoalsDialogProps) {
  const [open, setOpen] = useState(false);
  const { setNotificationMessage } = useNotification()

  const handleConfirm = () => {
    handleClearGoals();
    setNotificationMessage("All saving goals successfully cleared")
    setOpen(false);
  };
      ...
      {open &&
        createPortal(
          <Dialog title={'Please confirm'} width={300}>
            <p style={{ margin: '25px', textAlign: 'center' }}>Are you sure you want to clear all savings goals?</p>
